    ...
    async function importOnePath(
      ...
      try {
        ...
        if (result.status === "imported") {
          ...
          // v0.27 split-hash fast-path. Body unchanged so no new chunks, but
          // frontmatter-derived links/timeline (e.g. `key_people:`,
          // `attendees:`, `related:`) may have changed. Include the slug in
          // pagesAffected so post-sync extractLinksForSlugs /
          // extractTimelineForSlugs run; otherwise the graph drifts on
          // mapper-only edits.
          pagesAffected.push(result.slug);
