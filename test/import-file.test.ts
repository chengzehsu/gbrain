...
function mockEngine(overrides: Partial<Record<string, any>> = {}): BrainEngine {
  ...
  const engine = new Proxy({} as any, {
    get(_, prop: string) {
      ...
      if (prop === "getTags")
        return overrides.getTags || (() => Promise.resolve([]));
      if (prop === "getPage")
        return overrides.getPage || (() => Promise.resolve(null));
      // transaction: just call the fn with the same engine (no real DB transaction in tests)
      if (prop === "transaction")
        return async (fn: (tx: BrainEngine) => Promise<any>) => fn(engine);
      return track(prop);
    },
  });
  return engine;
...
`,
    ...
    const engine = mockEngine();
    const result = await importFile(engine, filePath, "concepts/from-path.md", {
      noEmbed: true,
    });
...
`;
  ...
  test("v0.27 split-hash fast-path: frontmatter-only change skips chunk + embed", async () => {
    // Body unchanged, frontmatter changed (e.g. notion-sync mapper added a
    // new `tier:` field). Should hit fast-path: no upsertChunks, no embed,
    // status='frontmatter_only'.
    const { createHash } = await import("crypto");
    const { parseMarkdown } = await import("../src/core/markdown.ts");
    const content = `---
type: concept
title: Frontmatter-Only Change
tier: 1
---

Body never changed.
`;
    const parsed = parseMarkdown(content, "concepts/fm.md");
    const bodyHash = createHash("sha256")
      ...
      .digest("hex");
    // existing has matching body_hash but stale content_hash (different frontmatter)
    const engine = mockEngine({
      getPage: () =>
        Promise.resolve({
    ...
    const result = await importFromContent(engine, "concepts/fm.md", content, {
      noEmbed: true,
    });
    expect(result.status).toBe("frontmatter_only");
    expect(result.chunks).toBe(0);

    const calls = (engine as any)._calls;
    // Page row IS written (frontmatter + hashes update)
    const putCalls = calls.filter((c: any) => c.method === "putPage");
    expect(putCalls.length).toBe(1);
    expect(putCalls[0].args[1].body_hash).toBe(bodyHash);
    expect(putCalls[0].args[1].content_hash).not.toBe("old-content-hash");
    // No chunks / tags / links / embed work
    expect(calls.find((c: any) => c.method === "upsertChunks")).toBeUndefined();
    expect(calls.find((c: any) => c.method === "deleteChunks")).toBeUndefined();
    expect(calls.find((c: any) => c.method === "addTag")).toBeUndefined();
    expect(calls.find((c: any) => c.method === "removeTag")).toBeUndefined();
    expect(calls.find((c: any) => c.method === "addLink")).toBeUndefined();
  });

  test("v0.27 split-hash: existing row without body_hash falls through to full path", async () => {
    // Pre-v0.27 rows have body_hash=null. First subsequent put must hit the
    // full path (chunk + embed) so body_hash gets populated for next time.
    const content = `---
type: concept
title: Pre-v0.27 row
---

Body content for the first re-sync.
`;
    const engine = mockEngine({
      getPage: () =>
        Promise.resolve({ content_hash: "stale", body_hash: null }),
    });
    ...
    const calls = (engine as any)._calls;
    // Full path: chunks AND putPage with body_hash set
    expect(calls.find((c: any) => c.method === "upsertChunks")).toBeDefined();
    const putCall = calls.find((c: any) => c.method === "putPage");
    expect(putCall).toBeDefined();
    expect(typeof putCall.args[1].body_hash).toBe("string");
    expect(putCall.args[1].body_hash.length).toBe(64); // sha256 hex
  });

  test("v0.27 split-hash: tag change does NOT hit fast-path (tags are in body_hash)", async () => {
    // Tag reconciliation lives inside the chunks transaction. If we let a
    // tag-only change skip via the fast path, tag state would drift. Verify
    // tag changes fall through to the full path.
    const { createHash } = await import("crypto");
    const { parseMarkdown } = await import("../src/core/markdown.ts");
    const content = `---
type: concept
title: Same body, new tag
tags: [new-tag]
---

Body content.
`;
    const parsed = parseMarkdown(content, "concepts/tagchange.md");
    // Compute body_hash WITHOUT the new tag (i.e. old tag set)
    const oldBodyHash = createHash("sha256")
      .update(
        JSON.stringify({
          title: parsed.title,
          type: parsed.type,
          compiled_truth: parsed.compiled_truth,
          timeline: parsed.timeline || "",
          tags: ["old-tag"].sort(), // different from current parsed.tags
        }),
      )
      .digest("hex");
    ...
    const engine = mockEngine({
      getPage: () =>
        Promise.resolve({ content_hash: "stale", body_hash: oldBodyHash }),
      getTags: () => Promise.resolve(["old-tag"]),
    });

    const result = await importFromContent(
      engine,
      "concepts/tagchange.md",
      content,
      { noEmbed: true },
    );
    expect(result.status).toBe("imported"); // NOT 'frontmatter_only'

    const calls = (engine as any)._calls;
    expect(calls.find((c: any) => c.method === "addTag")).toBeDefined();
    expect(calls.find((c: any) => c.method === "removeTag")).toBeDefined();
  });
...
`,
    ...
    const ctChunks = chunks.filter(
      (c: any) => c.chunk_source === "compiled_truth",
    );
...
`,
  ...
  test("uses UTF-8 byte length, not JS string length, for the size check", async () => {
    ...
    const engine = mockEngine();
    const result = await importFromContent(engine, "emoji-slug", bigContent, {
      noEmbed: true,
    });
  ...
  test("accepts in-memory content just under MAX_FILE_SIZE", async () => {
    ...
    const engine = mockEngine();
    const result = await importFromContent(engine, "borderline-slug", content, {
      noEmbed: true,
    });
  ...
  test("assigns sequential chunk_index values", async () => {
    ...
    const longText = Array(50)
      .fill("This is a sentence that adds length to the content.")
      .join(" ");
    writeFileSync(
      filePath,
      `---
